...
const SendTransaction = () => {
  ...
  return (
    <Card>
      <CardHeader id="send-transaction">Cast your vote</CardHeader>
      <a href={getFaucetUrl()} target="_blank" rel="noreferrer">
        <FormButton onClick={() => null} disabled={false}>
          Click to request your tokens{" "}
          <img src={Link} alt="link-icon" style={{ marginLeft: "3px" }} />
        </FormButton>
      </a>
      <Divider />
      <FormInput
        ...
        placeholder="Receiving Team's Address"
      />
      {toAddressError ? <ErrorText>Invalid address</ErrorText> : null}
      <FormInput
        value={amount}
        onChange={(e: any) => setAmount(e.target.value)}
        placeholder={`Amount of #GAMEON tokens`}
      />
      {amountError ? (
